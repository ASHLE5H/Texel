...
const BuyCredit = () => {
  ...
  const initPay = async (order)=>{            //takes order details for payment 
   try{
     const options = {
  ...
  const paymentRazorpay = async(planId)=>{   
      ...
      const {data} = await axios.post(backendUrl + '/api/user/pay-razor' , {planId} , {headers:{token}});    //sends plan selected by user to backend and gets order details

      if(data.success){
        initPay(data.order)     //calls initPay and passes order details
      }
      ...
      <div className='flex flex-wrap justify-center gap-6 text-left'>
        {plans.map((item, index) => (
          <div key={index} className='bg-white drop-shadow-sm border rounded-lg py-12 px-8 text-gray-600 hover:scale-105 transition-all duration-500'>
            <img width={60} className='-ml-2' src={assets.logo_icon} alt="" />
